...
async def handle_help(websocket):
    ...
    # MQ 命令
    help_text += "\n\n📨 MQ Topic 管理命令:\n"
    help_text += "  环境参数: dev, test, uat (默认 test) — 统一放在最后\n"
    help_text += "  \n"
    help_text += "  • /mq-list [关键词] [env]\n"
    help_text += "    列出所有 Topic，支持模糊搜索\n"
    help_text += "    示例: /mq-list               # test 环境，列出所有\n"
    help_text += "    示例: /mq-list cargo         # test 环境，搜索包含 cargo\n"
    help_text += "    示例: /mq-list nt uat        # uat 环境，搜索包含 nt\n"
    help_text += "  \n"
    help_text += "  • /mq-exists <topic> [env]\n"
    help_text += "    检查 Topic 是否存在\n"
    help_text += "    示例: /mq-exists test%nt_cargo\n"
    help_text += "    示例: /mq-exists test%nt_cargo uat\n"
    help_text += "  \n"
    help_text += "  • /mq-last <topic> [env]\n"
    help_text += "    查询最新一条消息的完整消息体\n"
    help_text += "    示例: /mq-last test%nt_cargo\n"
    help_text += "  \n"
    help_text += "  • /mq-create <topic> [env] — 创建 Topic\n"
    help_text += "  • /mq-delete <topic> [env] — 删除 Topic\n"
    help_text += "  \n"
    help_text += "  • /mq-send <topic> <JSON> [env]\n"
    help_text += "    发送 JSON 消息到 Topic\n"
    help_text += "    示例: /mq-send test%nt_cargo '{\"name\":\"张三\"}'\n"
    help_text += "  \n"
    help_text += "  • /mq-recent <topic> [分钟] [env]\n"
    help_text += "    查询最近 N 分钟的消息\n"
    help_text += "    示例: /mq-recent test%nt_cargo 10\n"
    help_text += "  \n"
